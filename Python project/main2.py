...
class BaseButton:
    def __init__(self, root, app, image_path, x, y):
        ...
        self.button_img = ImageTk.PhotoImage(Image.open(image_path).resize((80, 80), Image.LANCZOS))
...
class App:
    def __init__(self, root):
        ...
        self.btPlay_img = PlayButton(self.window, self, "Python project/Images/play.png", 250, 250)
        self.btPause_img = PauseButton(self.window, self, "Python project/Images/pause.png", 350, 250)
        self.btStop_img = StopButton(self.window, self, "Python project/Images/stop.png", 450, 250)
        self.btNext_img = NextButton(self.window, self, "Python project/Images/next.png", 550, 250)
        self.btPrev_img = PrevButton(self.window, self, "Python project/Images/prev.png", 150, 250)
        ...
        self.album_art_label.place(x=30, y=250, width=150, height=150)
    ...
    def add_folder(self):
        ...
        if folder_path:
            ...
            if songs:
                for song in songs:
                    song_name = os.path.basename(song)
                    self.directory_list.append({'path': folder_path + '/', 'song': song_name})
                    self.song_list.insert(END, song_name)
                self.song_list.select_set(0)
            else:
    ...
    def display_album_art(self, album_art):
        if album_art is not None:
            ...
            img = img.resize((150, 150), Image.LANCZOS)  # Resize the album art as needed
            ...
            # Handle the case where there's no album art for the song
            # You can display a default image or a message here.
            pass
...
class SongDuration:
    def __init__(self, app):
        self.app = app
        self.song_duration_bar = Label(self.app.window, text="Song Duration", font=("Arial", 17, "bold"), fg="white", bg="#141414", width=25)
    ...
    def update_song_duration_label(self, current_time):
        ...
        self.song_duration_bar.config(text=f"Time is: {self.format_time(current_time)} of {self.format_time(song_length)}")
